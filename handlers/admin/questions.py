...
async def process_questions(message: Message):
    ...
    print( requests.get('http://localhost:8000/bot/question/').json())
    ...
    if len(questions) == 0:
        ...
        for question in questions:
            ...
            await message.answer(f"Chat_id : {question['user_id']}\n{question['question']}", reply_markup=markup)
...
async def process_send_answer(message: Message, state: FSMContext):
    ...
    async with state.proxy() as data:
        ...
        answer = data['answer']
        cid = data['cid']
        question = requests.put(f'http://localhost:8000/bot/question/{cid}/', json={'answer': answer,"is_answered":True}).json()
        print(question)
